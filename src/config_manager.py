...
class ConfigManager:
    ...
    def _load_config_file(self):
        ...
        if not os.path.isfile(self.config_file_path):
            get_logger().error("Config file not found: %s", self.config_file_path)
            raise ConfigLoadingError(f"Config file not found: {self.config_file_path}")

        with open(self.config_file_path, "r", encoding="utf-8") as file:
            config = json.load(file)
            get_logger().info("Config file loaded from '%s'.", self.config_file_path)
            return config

    def _load_json_data(self, file_path):
        """Loads JSON data from a file."""
        if not os.path.isfile(file_path):
            get_logger().error("JSON file not found: %s", file_path)
            raise ConfigLoadingError(f"JSON file not found: {file_path}")

        with open(file_path, "r", encoding="utf-8") as file:
            data = json.load(file)
            get_logger().info("JSON data loaded from '%s'.", file_path)
            return data
    ...
    def _validate_file_path(self, path, description):
        ...
        if not os.path.isfile(path):
            get_logger().error("%s not found: %s", description, path)
    ...
    def _validate_directory_path(self, path, description):
        ...
        if not os.path.isdir(path):
            get_logger().error("%s does not exist: %s", description, path)
    ...
    def _load_nested_json_data(self):
        ...
        get_logger().info("Preprocessing config data loaded.")

        self.config["splitting"] = self._load_json_data(self.config["splitting"])
        get_logger().info("Splitting config data loaded.")

        self.config["training"] = self._load_json_data(self.config["training"])
        get_logger().info("Training config data loaded.")
